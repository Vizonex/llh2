...
def build_meson():
    ...
    if not os.path.exists("build-tmp"):
        subprocess.Popen(
            f"meson setup build-tmp --buildtype {'release' if '--debug' not in sys.argv else 'debug'}",
            cwd=Path(__file__).parent,
        ).wait()

    # we will save an execution step by using cwd argument aka current working directory to help
    subprocess.Popen(
        "meson compile --verbose", cwd=os.path.join(os.getcwd(), "build-tmp")
    ).wait()
...
def build_archive():
    ...
    with tarfile.open(outpath / (RELEASE_NAME + ".tar.xz"), "w:xz") as w:
        ...
        w.add(cwd / "src", "src")
        w.add(cwd / "LICENSE", "LICENSE")
        w.add(cwd / "meson.build", "meson.build")
        w.add(cwd / "llh2.pxd", "llh2.pxd")  # Cython bindings if anyone wants them...
        w.add(cwd / "CMakeLists.txt", "CMakeLists.txt")
