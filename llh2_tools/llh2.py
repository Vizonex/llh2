...
from .abstract import IntField, LLExt, MatchWrap, SpanWrap, UIntField
...
class Priority(AbstractParser):
    def build_start(self, end: Node) -> Node:
        ...
        start = p.invoke(
            ...
            dep_id.node.skipTo(
                p.invoke(p.code.match("llh2__post_dependency_id")).otherwise(
                    dep_id.on_complete.invoke_pausable(dep_id.err, stream_weight)
                )
            )
        )
...
class HeadersFrame(AbstractParser):
    ...
    def build(self, end: Node):
        ...
        body.otherwise(
            ...
            ).otherwise(self.error(Error.INVALID_LENGTH, "invalid length for headers"))
        )
        # return from root
        return padding
...
class PushPromiseFrame(Padding):
    ...
    def build(self, end: Node) -> Node:
        ...
        return self.start(self.on_promise_stream_id.consume(treat_as_dataframe))
...
class AltSvcFrame(AbstractParser):
    ...
    def __init__(self, llparse: LLParse, callbacks={}, fields={}):
        ...
        self.before_field_start = llparse.code.match("llh2__before_field_start")
    ...
    def build(self, end: Node) -> Node:
        ...
        before_field.otherwise(
            p.invoke(self.before_field_start, {0: origin}).otherwise(
                self.error(Error.INVALID_LENGTH, "ALTSVC FRAME had invalid length")
            )
        )
        ...
        start.otherwise(self.origin_length.consume(before_field))
        return start
